 ...
 *   - every answer scaffold's evidenceMap cites REAL profile-fact ids only
 *     (graph nodes are advisory context and never valid evidence refs);
...
- Every answer scaffold's evidenceMap[].factRef must be a REAL id from the provided PROFILE FACTS only. Graph nodes are advisory context and their ids are NEVER valid evidenceMap.factRef values. Never invent an id.
...
export function buildInterviewerUserPrompt(input: InterviewPrepInput): string {
  ...
  return [
    ...
    `\nGRAPH NODES (advisory context only — NEVER valid ids for evidenceMap.factRef):\n${graph}`,
