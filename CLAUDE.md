...
| --- | --- | --- | --- |
| 2026-08-09 | FM2 Step 0 — First-Run Identity | **COMPLETE** | Added explicit backend-owned onboarding union/timestamp, deploy-only migration + existing-user backfill, verified-principal `POST /v1/me/bootstrap`, designed missing-row semantics, exhaustive app/onboarding guards with visible dependency recovery, live Postgres concurrency/migration/deletion acceptance, and real first-run Playwright. Workspace 1007, eval CI 216, DB integration 20, API integration 51, route axe 21, and Playwright 3 passed; production build and `make verify` green with no test skips. Existing onboarding placeholder only; import/review/reflect-back/autonomy/opportunity UI and completion write remain deferred. |
