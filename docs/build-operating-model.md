...
|---|---|---|---|---|---|
| 2026-08-09 | **FM2 Step 0 — first-run identity + authoritative onboarding contract.** Backend-owned nullable completion timestamp, migration backfill, explicit idempotent/concurrent Green bootstrap, intentional missing-row semantics, exhaustive server routing + inverse onboarding guard, dependency recovery, contract-backed fixtures, real Postgres/API/Playwright acceptance. No onboarding completion write or later FM2 UI/content implemented. | Cline | ✅ N=8 bootstrap concurrency; DB 20; API 51; workspace 1007; eval 216; guarantees 6; fixtures 4; axe 21; Playwright first-run + existing Twin + dependency recovery 3/3; production build | Production Clerk remains fail-closed FM1 debt; onboarding completion endpoint/criteria and all onboarding content are deferred. | ✅ full parity + `make verify` + diff check; no skips |
