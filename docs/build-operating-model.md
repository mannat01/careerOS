...
- **M06 §4A `isMaterialChange` duplication.** The material-change predicate is currently authored twice — once in `packages/cie/planner/src/io.ts` (used by the planner's own `decideReplan`) and once in `apps/api/src/modules/cie/plan.handlers.ts` (used by the handler's sub-threshold short-circuit before ever invoking the planner). Both gates must agree or the handler could write a plan the planner would have refused (or short-circuit a change the planner would have regenerated on). Follow-up: lift `isMaterialChange` into a tiny shared package (e.g. `@careeros/cie-planner-gate`) that both the pure planner service and the app-side handler import — deleting the duplication and letting the compiler enforce parity. Chosen now to avoid a madge cycle (`apps/api` already imports `@careeros/cie-planner`; a shared package removes the temptation to reach the other way).
